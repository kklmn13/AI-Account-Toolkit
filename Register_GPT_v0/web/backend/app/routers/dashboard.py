...
def get_dashboard(username: str = Depends(get_current_user)):
    ...
    with get_db() as conn:
        ...
        sora_available_count = c.fetchone()[0]
        c.execute(
            """SELECT COUNT(*) FROM sora_video_tasks
               WHERE task_id NOT LIKE 'lease_%'
                 AND normalized_status = 'succeeded'
                 AND date(COALESCE(succeeded_at, updated_at), 'localtime') = date('now', 'localtime')"""
        )
        today_generated_videos = c.fetchone()[0]
        c.execute(
    ...
    return {
        ...
        "sora_available_count": sora_available_count,
        "today_generatable_videos": sora_available_count,
        "today_generated_videos": today_generated_videos,
