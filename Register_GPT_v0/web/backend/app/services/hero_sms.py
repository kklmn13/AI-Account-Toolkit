...
def _is_service_not_available_error(error: Optional[str]) -> bool:
    text = str(error or "")
    return any(token in text for token in SERVICE_NOT_AVAILABLE_TOKENS)


def _country_candidates_from_prices(
    base_url: str,
    api_key: str,
    service: str,
    max_price: Optional[float] = None,
    ...
    prices = get_prices(base_url, api_key, service=service)
    candidates = []
    if isinstance(prices, dict):
        for country_id, val in prices.items():
            if not isinstance(val, dict):
                continue
            info = val.get(service) if isinstance(val.get(service), dict) else None
            if not info:
                continue
            try:
                count = int(info.get("count") or info.get("physicalCount") or 0)
            ...
            if max_price is not None and cost > max_price:
                continue
            candidates.append((cost, -count, cid))
    candidates.sort()
    return [cid for _, _, cid in candidates[:limit]]
...
def get_number_auto(
    ...
    """
    申请号码，优先使用传入 country。
    若 country=0 命中 Hero-SMS 的 SERVICE_NOT_AVAILABLE，则自动按价格/库存挑选具体国家重试。
    """

    def _try_one(country_code: int) -> Optional[Dict[str, Any]]:
        result = get_number(
            base_url,
            api_key,
            service,
        ...
        if fallback and not fallback.get("error"):
            fallback["country"] = country_code
            return fallback
        err = ""
        if isinstance(result, dict) and result.get("error"):
            err = str(result.get("error"))
        elif isinstance(fallback, dict) and fallback.get("error"):
            err = str(fallback.get("error"))
        return {"error": err or "无可用号码"}

    first = _try_one(country)
    if first and not first.get("error"):
        return first
    if country != 0 or not _is_service_not_available_error((first or {}).get("error")):
        return first

    for country_code in _country_candidates_from_prices(
        base_url,
        api_key,
        service,
        max_price=max_price,
        ...
        if country_code == country:
            continue
        result = _try_one(country_code)
        if result and not result.get("error"):
            result["auto_country_fallback"] = True
            return result
    return first
